package eccrm.base.attachment.service.impl;

import com.ycrl.core.beans.BeanWrapBuilder;
import com.ycrl.core.beans.BeanWrapCallback;
import com.ycrl.core.pager.PageVo;
import eccrm.base.attachment.bo.AttachmentBo;
import eccrm.base.attachment.dao.AttachmentDao;
...
public class AttachmentServiceImpl implements AttachmentService, BeanWrapCallback<Attachment, AttachmentVo> {
    ...
    public void save(Attachment attachment) {
        ...
        if (type == null) {
            throw new RuntimeException("不合法的附件对象!没有指定附件类型!");
        }
        if (type.equals(AttachmentType.FILE.getValue())) {
            // 设置path
        ...
        } else if (type.equals(AttachmentType.URL.getValue())) {

        } else {
            throw new RuntimeException("不合法的附件类型:" + type);
        }
        // 设置为临时状态
    ...
    public void bind(String businessId, String[] ids) {
        ...
        if (attachments != null && !attachments.isEmpty()) {
            for (Attachment attachment : attachments) {
                ...
                if (AttachmentType.FILE.getValue().equals(attachment.getAttachmentType())) {
                    // 将附件从临时目录转移到正式目录
