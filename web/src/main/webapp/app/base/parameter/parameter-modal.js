...
(function (angular, window) {
    var app = angular.module('eccrm.base.parameter.modal', [
        'eccrm.base.param',
        'eccrm.angular',
        'eccrm.angularstrap',
    ...
    var common_type = function (config, options, callback) {
        ...
        var checkName = function (defer) {
            var pid = CommonUtils.parse($scope, 'parameterType.parent.id');
            if (originalName === $scope.parameterType.name) {
                defer.resolve(true);
                return true;
            }
            var result = ParameterType.hasName({id: pid, name: CommonUtils.encode($scope.parameterType.name)});
            AlertFactory.handle($scope, result, function (data) {
    ...
    var common_item = function (config, options, callback) {
        ...
        var checkValue = function () {
            ...
            var result = ParameterItem.hasValue({type: $scope.beans.type, value: CommonUtils.encode($scope.beans.value)});
            AlertFactory.handle($scope, result, function (data) {
